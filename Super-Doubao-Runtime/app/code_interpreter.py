...
from app.types.messages import CodeInterpreterRequest, CodeInterpreterResult, CodeInterpreterResponse, EmptyCodeInterpreterResult, CIFileInfo
from app.analyze.analyze_timeout_v2 import static_analyze_code_async, CodeAnalyzeResult, TimeoutReason
from concurrent.futures import ProcessPoolExecutor
import multiprocessing
from app.types.error_code import ErrorCode
...
async def get_or_init_kernel(kernel_id: str, check_channel=False, cwd=None) -> KernelManager:
    ...
    if cwd:
        kwargs["cwd"] = cwd
    await _MULTI_KERNEL_MANAGER.start_kernel(**kwargs)
...
async def interrupt_kernel(kernel_id: str):
    try:
        ...
        logger.info(f"kernel_id:{kernel_id} interrupted successfully")
    except Exception as e:
...
async def run_code_with_analyze_timeout(
    ...
    # 1. 初始化kernel
    try:
        km = await get_or_init_kernel(req.kernel_id, True)
        kc = km.client()
    except Exception as e:
...
async def execute_code(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"kernel_id:{kernel_id} execute timeout, interrupt kernel...")
